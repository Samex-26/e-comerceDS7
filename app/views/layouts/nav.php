    ...
    <div class="container" style="max-width: 1320px;">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav ms-auto align-items-center gap-1">
                <li class="nav-item"><a class="nav-link" href="<?= BASE_URL ?>idioma/cambiar/es">ES</a></li>
                <li class="nav-item"><a class="nav-link" href="<?= BASE_URL ?>idioma/cambiar/en">EN</a></li>
                ...
                <?php
                    $esClienteSesion = isset($_SESSION['id_usuario'])
                        && ($_SESSION['rol'] ?? '') === 'cliente'
                        && (int) ($_SESSION['activo'] ?? 1) === 1
                        && (int) ($_SESSION['bloqueado'] ?? 0) === 0;
                    $cartCount = 0;
                    if ($esClienteSesion && isset($_SESSION['carrito'])) {
                        foreach ($_SESSION['carrito'] as $item) {
                            $cartCount += (int) ($item['cantidad'] ?? 0);
                        }
                    }
                ?>
                <?php if ($esClienteSesion): ?>
                <li class="nav-item">
                ...
                <?php if (isset($_SESSION['id_usuario'])): ?>
                    ...
                    <?php endif; ?>
                    <li class="nav-item"><form method="POST" action="<?= BASE_URL ?>auth/logout"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>"><button class="nav-link btn py-1 px-3 fw-semibold" style="border: 1px solid #e2e8f0; color: #1e293b; border-radius: 0.375rem;" type="submit"><?= htmlspecialchars($lang['cerrar_sesion']) ?></button></form></li>
                <?php else: ?>
